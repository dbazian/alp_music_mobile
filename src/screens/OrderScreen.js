  ...
  if (orders.length === 0) {
    return (
      <Gradient>
        ...
        <MainButton onPress={() => props.navigation.navigate({ routeName: "User" })} name={"Back"} />
      </Gradient>
    );
  }
  ...
  return (
    <Gradient>
      <FlatList
        ...
        renderItem={(itemData) => (
          <OrderItem items={itemData.item.items} date={itemData.item.date} totalAmount={itemData.item.totalAmount} />
        )}
      />
